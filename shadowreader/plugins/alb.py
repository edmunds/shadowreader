...
from libs.s3 import put_on_s3, _generate_key_prefix_base_on_time, delete_tmp_file, fetch_file_names_on_s3
from classes.mytime import MyTime
from datetime import datetime
from typing import Tuple, Match

s3cl = boto3.client('s3')
elb_regex = '(?P<type>[\S]+) (?P<timestamp>[\S]+) (?P<elb>[\S]+) ' \
            '(?P<client>[\S]+):(?P<client_port>[\S]+) (' \
            '?P<target>[\S]+):(?P<target_port>[\S]+) ' \
            '(?P<request_processing_time>[\S]+) (' \
            '?P<target_processing_time>[\S]+) ' \
            '(?P<response_processing_time>[\S]+) (?P<elb_status_code>[\S]+) (' \
            '?P<target_status_code>[\S]+) ' \
            '(?P<received_bytes>[\S]+) ' \
            '(?P<sent_bytes>[\S]+) (?P<request>\".*\") (' \
            '?P<user_agent>\".*\") (?P<ssl_cipher>[\S]+) ' \
            '(?P<ssl_protocol>[\S]+) (?P<target_group_arn>[\S]+) (' \
            '?P<trace_id>[\S]+) (?P<domain_name>[\S]+) ' \
            '(?P<chosen_cert_arn>[\S]+)'
...
def _parse_line(line: Match[str]) -> dict:
    ...
    timestamp = datetime.strptime(
        timestamp[:19], '%Y-%m-%dT%H:%M:%S')  # convert to datetime obj
...
def _filter_payload_for_get_requests(payload: dict, app: str) -> dict:
    for timestamp, val in payload[app].items():
        payload[app][timestamp] = list(
            filter(lambda x: x['req_method'] == 'GET', val))
    return payload
...
def _download_and_parse_s3_data(files: list, elb_logs_bucket: str,
                                app: str) -> dict:
    lines = []
    for f in files:
