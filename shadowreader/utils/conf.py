...
class Plugins:
    def __init__(self):
        ...
        self.sr_config = self._identify_keys_w_stage(sr_config,
                                                     self.env_vars['stage'])
    ...
    def _load_plugins(self):
        ...
        plugins_conf = self._parse_plugins(plugins_conf, stage,
                                           plugins_location)
    ...
    def _parse_plugins(self, plugins_conf: dict, stage: str,
                       plugins_location: str):
        """ Parse the plugins config by mapping plugin names to module paths """
        plugins_conf = self._identify_keys_w_stage(
            conf=plugins_conf, stage=stage)
